      ...
      init: () => {
         let callback = (res) => {
            ...
            if (!(new Audio().canPlayType('audio/ogg; codecs="vorbis"'))) {
               var inputAudiu = document.getElementsByName('soundNotification')[0];
               inputAudiu.checked = false;
               inputAudiu.disabled = true;
               var warnMsg = 'Your browser does not support the .ogg audio file'
               inputAudiu.title = warnMsg;
               inputAudiu.parentElement.setAttribute("tooltip", warnMsg);
            }
